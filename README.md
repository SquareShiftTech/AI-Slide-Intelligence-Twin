...
4.  **Obtain Google API Credentials:**
    ...
    *   On the "Scopes" page during consent screen setup, click "ADD OR REMOVE SCOPES". Search for and add the following scopes:
        ...
        *   `https://www.googleapis.com/auth/drive.readonly` (optional; for listing files)
        *   `https://www.googleapis.com/auth/drive` (required if you use **template presentations**: create from a copy of an existing deck to keep theme/layout)
...
6.  **Configure Credentials and Command in MCP Settings:**
    ...
    ```
    Replace `/path/to/google-slides-mcp/build/index.js` with the actual path to the compiled server index file on your system. Replace `YOUR_CLIENT_ID`, `YOUR_CLIENT_SECRET`, and `YOUR_REFRESH_TOKEN` with the actual values you obtained. The MCP runner will inject these values into the server's environment when it starts. Optional: `GOOGLE_SLIDES_TEMPLATE_ID` — Google Slides presentation ID to use as the default template for all created decks (default is built-in; set this to use your own template).
...
The server starts as an **HTTP server** (for Elastic Agent Builder, Elastic Cloud, or any remote client). It listens on **port 3000** by default (override with the `PORT` environment variable). You should see: `Google Slides MCP server running at http://localhost:3000`.

### HTTP API (create presentation from agent content)

**POST** `http://localhost:3000/slides`

Send slide content as JSON. The server creates a Google Slides presentation and returns an **edit URL** for the agent to show the user.
...
- `slides` (array, required): At least one slide. Each slide can have `title`, `subtitle`, `body`, `bullets` (array of strings), and `notes`. All fields except the first slide’s content are optional.
- **Default template:** All presentations are created from a **default template** (Drive copy). The template must have **at least 3 slides**: (1) **title slide** — only the **presentation title** (the request `title`) is placed here; (2) **content layout slide** — duplicated for each item in `slides` and filled with that slide’s title/body/bullets; (3) **thank you slide** (last) — left unchanged. So the first slide shows only the deck title; all content starts from the duplicated slide 2 layout. Override the default with `GOOGLE_SLIDES_TEMPLATE_ID` or `templatePresentationId` in the request.
...
The agent can use `editUrl` so the user can open and edit the deck in the browser. Use `presentationId` for any follow-up API calls (e.g. batch updates).

**Health check:** `GET http://localhost:3000/health` returns `{ "status": "ok", "service": "google-slides-mcp" }`.

### MCP tools for Elastic Agent Builder

The server supports the [Model Context Protocol (MCP)](https://modelcontextprotocol.io) over HTTP so the [Elastic MCP connector](https://www.elastic.co/docs/reference/kibana/connectors-kibana/mcp-action-type) can list and call tools.

**Option A — Single MCP endpoint (recommended)**  
Set the connector **URL** to your server’s **MCP endpoint**:

- **URL:** `https://<your-server>/mcp` (e.g. `https://your-ngrok-url.ngrok-free.app/mcp`)

Elastic will send **POST** requests with a JSON-RPC 2.0 body. The server handles:

- `method: "initialize"` — server info and capabilities  
- `method: "tools/list"` — returns `{ "result": { "tools": [...] } }`  
- `method: "tools/call"` — `params.name` and `params.arguments`; returns `{ "result": { "content", "isError" } }`

**Option B — Separate paths**  
If your connector uses different URLs for “list tools” and “call tool”:

- **List tools:** `https://<your-server>/tools` (GET or POST). If Elastic POSTs a JSON-RPC body with `method: "tools/list"`, the response is JSON-RPC; otherwise plain `{ "tools": [...] }`.
- **Call tool:** `https://<your-server>/tools/call` (POST, body: `{ "name", "arguments" }`) for plain JSON, or use the single **POST /mcp** endpoint with `method: "tools/call"`.

**Available tool names:** `create_presentation`, `create_presentation_from_content`, `get_presentation`, `batch_update_presentation`, `get_page`, `summarize_presentation`.
